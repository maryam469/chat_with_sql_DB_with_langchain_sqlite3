...
"""
        ...
        mysql_host = st.text_input(
            "Host", 
            value=os.getenv("MYSQLHOST", "127.0.0.1"),
            help="Usually 127.0.0.1 (localhost) or Railway host."
        )
        mysql_port = st.text_input(
            "Port", 
            value=os.getenv("MYSQLPORT","3306"), 
            help="Default port is 3306."
            )
        mysql_user = st.text_input(
        "Username", 
        value=os.getenv("MYSQLUSER","root"), 
        help="Provided by DBA or Railway."
        )
        mysql_password = st.text_input(
            "Password", 
            type="password", 
            value =os.getenv("MYSQLPASSWORD", ""),
            help="Keep it safe."
            )
        mysql_db = st.text_input(
            "Database Name", 
            value=os.getenv("MYSQLDATABASE", "student"), 
            help="Target DB name."
            )
...
try:
    if DB_MODE.startswith("SQLite"):
        ...
        MYSQL_PORT = os.getenv("MYSQL_PORT", mysql_port)
...
with col2:
    sample_query = st.text_input(
        ...
        key="sample_query_input" 
    )
    if st.button("Send Sample Query"):
        st.session_state["user_query"] = sample_query
